...
Resource pins use nominal types: `task`, `repository`, `template`, `markdown`, `open_items`, and `decision_log`. Their JSON storage representation does not make them interchangeable. Template pins are purple, Markdown green, Task blue, Repository orange, Open Items yellow, and Decision Log pink; the connecting wire uses the same color. Plain JSON objects, arrays, and primitive values remain available as separate types. Type names remain visible alongside colors. Shipped 2.0.1 definitions adopt these types; older immutable versions remain readable.
