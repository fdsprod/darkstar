...
const palette: { type: WorkflowNodeType; label: string; description: string }[] = [
  {type:"workspace_prepare",label:"Prepare workspace",description:"Choose repository, checkout, base ref and worktree branch"},
  {type:"workspace_validate",label:"Validate workspace",description:"Run required checks in the connected workspace before Done"},
  { type: "implementation", label: "Implementation", description: "Change repository files and validate the result" },
...
export function WorkflowsPage() {
  ...
  const [markdownOwner,setMarkdownOwner]=useState<string>();
  ...
  const selectItem = (item: LibraryItem) => { setChatEpoch(value => value + 1); setParams({ item: itemKey(item), selection: undefined, view: item.kind === "draft" ? view : "structure" }); };
  ...
  function connectPorts(source: Port, target: Port) {
    if (document && source.kind === "execution" && source.nodeId === "$start" && target.kind === "execution") { const next = structuredClone(document); record(record(next.spec).routeDefaults).entry = target.nodeId; record(record(record(next.spec).nodes)[target.nodeId]).entry=true; changeDocument(next,"Start connected."); return; }
    if (!document || !portGraph) return;
    const error = connectionError(source, target); if (error) { setAnnouncement(error); return; }
    if (source.kind === "execution" && target.kind === "execution" && ["true","false"].includes(source.portId)) {
      const next=structuredClone(document), node=record(record(record(next.spec).nodes)[source.nodeId]);
      const transitions=Array.isArray(node.transitions)?node.transitions as JsonObject[]:[];
      const existing=transitions.find(t=>JSON.stringify(record(t.when).args??[]).includes('"literal":'+source.portId));
      if(existing)existing.to=target.nodeId;else transitions.push({id:source.nodeId+"_"+source.portId,to:target.nodeId,when:{op:"eq",args:[{ref:"output.passed"},{literal:source.portId==="true"}]}});
      node.transitions=transitions;changeDocument(next,"Branch connected.");return;
    }
  ...
  const chatTarget: ChatTarget = draft ? { kind: "draft", id: draft.id, revision: draft.revision } : selected?.kind === "installed" ? { kind: "version", name: selected.version.name, version: selected.version.version } : { kind: "new" };
  function receiveChat(event: ChatEvent) {
    if (event.kind === "draft") {
      const saved = event.payload;
      validationRequestRef.current++; previewRequestRef.current++; publishRequestRef.current++; semanticGenerationRef.current++; saveRequestRef.current++;
      currentDraftRef.current = { id: saved.id, revision: saved.revision, documentDigest: saved.documentDigest };
      setDraft(saved); setDocument(structuredClone(saved.document) as JsonObject); setLayout(structuredClone(saved.layout) as JsonObject);
      setPersistence({ kind: "clean", revision: saved.revision }); setValidation({ kind: "not_run" }); setPublish({ kind: "closed" }); setRoutePreview({ kind: "closed" });
      setLibrary(current => ({ ...current, drafts: [...current.drafts.filter(item => item.id !== saved.id), saved] }));
      setParams({ item: `draft:${saved.id}` }, true);
    } else if (event.kind === "validation") {
      const report = event.payload, current = currentDraftRef.current;
      if (!current || current.id !== report.draftId || current.revision !== report.revision || current.documentDigest !== report.documentDigest) return;
      const evidence = { draftId: report.draftId, revision: report.revision, documentDigest: report.documentDigest };
      setValidation(report.findings.length ? { kind: "invalid", ...evidence, findings: report.findings } : report.digest ? { kind: "valid", ...evidence, semanticDigest: report.digest } : { kind: "error", ...evidence, message: "Validation did not return a semantic digest." });
    } else if (event.kind === "conflict" && currentDraftRef.current?.id === event.payload.remote.id) {
      setPersistence({ kind: "conflict", revision: currentDraftRef.current.revision, remote: event.payload.remote });
      setValidation({ kind: "not_run" }); setPublish({ kind: "closed" });
    }
  }
