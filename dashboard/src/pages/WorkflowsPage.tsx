  ...
  return <div ref={pageRef} className="page workflows-page workflow-authoring-page" onKeyDown={(event) => { if (event.key === "Escape") { if (newOpen) setNewOpen(false); if (advancedOpen) setAdvancedOpen(false); if (routePreview.kind !== "loading") setRoutePreview({ kind: "closed" }); if (publish.kind !== "publishing" && publish.kind !== "closed") setPublish({ kind: "closed" }); setConnectFrom(undefined); select({ kind: "none" }); } }}>
    ...
    <div className="workflow-editor-shell">
      <aside className="workflow-editor-left" aria-label="Workflow library and node palette">
        ...
        {draft && <section className="node-palette" aria-labelledby="node-palette-title"><h2 id="node-palette-title">Add node</h2><label className="field"><span>Search nodes</span><input type="search" value={paletteQuery} onChange={(event) => setPaletteQuery(event.target.value)} /></label>{palette.filter((item) => `${item.label} ${item.description}`.toLowerCase().includes(paletteQuery.toLowerCase().trim())).map((item) => { const availability = nodeTypeAvailability(item.type, catalog); return <button type="button" key={item.type} draggable={availability.available} disabled={!availability.available} title={availability.message ?? "Click to add or drag onto the canvas"} onDragStart={(event) => { event.dataTransfer.setData("application/x-darkstar-workflow-node", item.type); event.dataTransfer.effectAllowed = "copy"; }} onClick={() => void add(item.type)}><strong>{item.label}</strong><span>{availability.message ?? item.description}</span></button>; })}</section>}
