...
import { ProjectRepositoriesPanel } from "./ProjectRepositoriesPanel";
import {
...
export function SettingsPage() {
  ...
  const [repositoryDraftOpen, setRepositoryDraftOpen] = useState(false);
  ...
  const hasDraft = editor.state !== "closed" || secretOpen || repositoryDraftOpen;
  ...
  return <div className="page settings-page">
    ...
    {tabs.map((item) => <div key={item.id} id={`settings-panel-${item.id}`} className="settings-panel" role="tabpanel" tabIndex={item.id === tab ? 0 : -1} aria-labelledby={`settings-tab-${item.id}`} aria-busy={refreshing} hidden={item.id !== tab}>
      {item.id === "projects" && <ProjectRepositoriesPanel selectedProjectId={scopeProjectId(scope)} refreshVersion={refreshVersion} onRegister={() => registerDialog.current?.showModal()} onDraftOpen={setRepositoryDraftOpen} onChanged={() => setRefreshVersion((value) => value + 1)} onSelect={(projectId) => {
        updateParams((next) => {
          next.set("scope", "project");
          next.set("projectId", projectId);
          next.delete("setting");
        }, editor.state !== "closed" || secretOpen);
      }} />}
