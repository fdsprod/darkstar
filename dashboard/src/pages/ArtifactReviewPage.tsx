...
export function ArtifactReviewWorkspace({ approvalId, embedded = false }: { approvalId: string; embedded?: boolean }) {
  ...
  return <div className="page review-page">
    ...
    <div className="review-workspace review-workspace--document">
      <details className="review-history-disclosure"><summary>Artifact versions</summary><RevisionRail revisions={revisions} selected={shownVersion ?? session.candidate.version} reviewed={new Set(orderedReviewSessions(history.sessions).map((item) => item.candidate.version))} onSelect={selectRevision} /></details>
      <section className="review-reader" tabIndex={-1} aria-label="Document">
        <header className="review-region-heading"><p className="eyebrow">Document</p><span>Exact safe representation</span></header>
...
async function submitAnnotationFeedbackSet(session: ReviewSession, representation: Extract<SafeTextState, { kind: "available" }> | undefined, overallInstruction: string, annotations: readonly FeedbackAnnotationDraft[], signal: AbortSignal): Promise<{ session: ReviewSession; set: FeedbackSetView }> {
  ...
  const instruction = overallInstruction.trim() || (annotations.length ? "Address the attached annotations in this document." : "");
