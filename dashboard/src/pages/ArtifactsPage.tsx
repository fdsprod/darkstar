...
export function ArtifactsPage() {
  ...
  return <div className="page artifacts-page">
    ...
    {notice && <AsyncPanel compact state={notice.includes("changed") ? "stale" : "success"} title={notice.includes("changed") ? "State changed" : "Operation complete"} message={notice} />}{error && <AsyncPanel compact state="error" title="Operation failed" message={error} />}
    ...
    {latest.length ? <div className="artifact-catalog">{latest.map((view) => <ArtifactCard key={view.artifact.artifactId} view={view} />)}</div> : <EmptyState kind={requestedTarget ? "filtered" : "empty"} title="No artifacts in this scope" message="Add a file or pasted note and bind it to this target. Stored evidence remains immutable and auditable." action={requestedTarget ? <button type="button" className="button" onClick={() => navigate("/artifacts")}>Clear filters</button> : <button type="button" className="button button--primary" onClick={() => dialog.current?.showModal()}>Add evidence</button>} />}
...
function IngestDialog({ refValue, initialTarget, onClose, onComplete }: { refValue: React.RefObject<HTMLDialogElement | null>; initialTarget?: { kind: ArtifactTargetKind; id: string }; onClose(): void; onComplete(message: string): Promise<void> }) {
  ...
  return <dialog ref={refValue} className="work-dialog artifact-ingest-dialog" aria-labelledby="artifact-ingest-title" onCancel={(event) => { if (submitting) event.preventDefault(); }} onClose={() => { if (!submitting) { setError(""); onClose(); } }}><form aria-busy={submitting} onSubmit={(event) => void submit(event)}><header className="work-dialog__header"><div><p className="eyebrow">Immutable source + exact target</p><h2 id="artifact-ingest-title">Add evidence</h2></div><button type="button" className="icon-button" aria-label="Close evidence ingestion" onClick={onClose}>×</button></header><p className="work-dialog__intro">The original bytes are stored as untrusted evidence. Ingestion and attachment are separate durable operations; impact assessment never changes a route by itself.</p>
    ...
    {error && <p className="form-error" role="alert">{error}</p>}<footer className="work-dialog__footer"><p className="dialog-draft-note">Closing discards unsaved changes.</p><button type="button" className="button" disabled={submitting} onClick={onClose}>Cancel</button><button type="submit" className="button button--primary" disabled={submitting}>{submitting ? (partial ? "Retrying attachment…" : "Storing & attaching…") : (partial ? "Retry attachment" : "Store & attach evidence")}</button></footer></form></dialog>;
}
