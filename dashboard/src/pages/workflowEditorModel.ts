...
function supportedSharedSections(node: JsonObject) {
  ...
  if (!hasOnlyKeys(node, ["displayName", "type", "entry", "terminal", "inputs", "outputs", "readiness", "definition", "prompt", "reasoning", "gate", "command", "approval", "call", "points", "routing", "implementation", "workspacePrepare", "workspaceValidate", "gitCommit", "gitPush", "createPR", "extension", "validators", "retry", "checkpoint", "transitionMode", "join", "permissions", "transitions"])) return false;
  if (node.prompt !== undefined) {
    const prompt = record(node.prompt);
    if (!["reasoning", "implementation"].includes(String(node.type)) || !hasOnlyKeys(prompt, ["id", "version", "digest"]) || ![prompt.id, prompt.version].every((value) => typeof value === "string" && /^[a-zA-Z0-9][a-zA-Z0-9._-]{0,127}$/.test(value)) || typeof prompt.digest !== "string" || !/^[0-9a-f]{64}$/.test(prompt.digest)) {
      return false;
    }
  }
