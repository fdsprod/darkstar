...
export function WorkflowPortCanvas({ graph, ports, selection, layout, onConnect, onFocus, onSelect, onLayout }: CommonProps & { selection: EditorSelection; layout: WorkflowLayout; onSelect(selection: EditorSelection): void; onLayout(layout: JsonObject, message: string): void }) {
  ...
  return <div className="workflow-port-graph" onKeyDown={(event) => { if (event.key === "Escape" && pending) { event.stopPropagation(); setPending(undefined); setMessage("Connection cancelled."); } }}><div className="port-canvas-toolbar" role="group" aria-label="Canvas controls">
    <button className="button button--compact" type="button" onClick={() => setViewport(bounds.x, bounds.y, fitZoom)}>Zoom to fit</button><button className="button button--compact" type="button" aria-label="Zoom in" onClick={() => setViewport(viewport.x, viewport.y, zoom * 1.25)}>+</button><button className="button button--compact" type="button" aria-label="Zoom out" onClick={() => setViewport(viewport.x, viewport.y, zoom / 1.25)}>−</button>
    <button className="button button--compact" type="button" onClick={() => onLayout({ ...autoLayoutGraph(layout, graph, ports.ports), viewport: undefined }, "Auto-layout applied; execution is unchanged.")}>Auto-layout</button>
    {pending && <button className="button button--compact" type="button" onClick={() => { setPending(undefined); setMessage("Connection cancelled."); }}>Cancel connection</button>}<span>Solid: execution · Dashed: data</span>
  </div><p className="sr-only" role="status">{message}</p>{message && <p className="port-message">{message}</p>}
  {ports.ports.some((port) => port.kind === "run_input") && <div className="run-input-ports" aria-label="Run input output ports">{ports.ports.filter((port) => port.kind === "run_input").map((port) => <button className="button button--compact" key={portKey(port)} type="button" onClick={() => choose(port)}>{portLabel(port)} →</button>)}</div>}
