  ...
  return (
    <div className="page detail-page run-detail-page">
      ...
      <header className="page-header detail-header run-detail-header">
        ...
        <div className="run-header-actions"><StatusPill status={view.run.status} />{controls.map((control) => <button className="button" type="button" key={control} disabled={Boolean(action)} onClick={() => void invoke(control)}>{action === control ? "Requesting…" : humanize(control)}</button>)}</div>
      </header>
