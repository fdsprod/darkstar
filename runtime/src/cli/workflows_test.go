...
func TestWorkflowCLIInstallListGraphAndPreviewJSON(t *testing.T) {
	...
	if authoringCatalog.SchemaVersion != 1 || len(authoringCatalog.NodeTypes) != 13 || authoringCatalog.Workflows.Status != workflow.ReferenceKnown || len(authoringCatalog.Workflows.Items) != 1 || authoringCatalog.Agents.Status != workflow.ReferenceUnavailable || authoringCatalog.Skills.Status != workflow.ReferenceKnown {
