...
func newFakeRunProvider(scenarioName, attemptID string, resume bool) (provider.Provider, error) {
	steps := []fake.Step{
		...
		fake.Emit(provider.Event{Sequence: 2, Kind: provider.EventMessageCompleted, Payload: json.RawMessage(`{"text":"deterministic fake-provider evidence"}`)}),
	...
	if scenarioName == runexecution.ScenarioRestart {
		steps = []fake.Step{
			fake.Emit(provider.Event{Sequence: 1, Kind: provider.EventTurnStarted, Payload: json.RawMessage(`{"phase":"before-restart"}`)}),
			fake.Pause(24 * time.Hour),
			fake.Emit(provider.Event{Sequence: 2, Kind: provider.EventMessageCompleted, Payload: json.RawMessage(`{"text":"resumed after daemon restart"}`)}),
			fake.Emit(provider.Event{Sequence: 3, Kind: provider.EventTurnCompleted, Payload: json.RawMessage(`{"phase":"completed"}`)}),
		}
		if !resume {
