...
func TestPublishedEnumsMatchTypedContract(t *testing.T) {
	...
	assertStringSet(t, schemaEnum(t, v3, "$defs", "node", "properties", "type", "enum"), []string{string(workflow.NodeWorkspacePrepare), string(workflow.NodeWorkspaceValidate), string(workflow.NodeReasoning), string(workflow.NodeImplementation), string(workflow.NodeGate), string(workflow.NodeCommand), string(workflow.NodeApproval), string(workflow.NodeSubworkflow), string(workflow.NodePointExecution), string(workflow.NodeRouting)})
