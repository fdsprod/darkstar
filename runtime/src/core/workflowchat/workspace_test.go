...
func TestWorkspaceExampleAndRequiredConnections(t *testing.T) {
	...
	for _, raw := range []string{strings.Replace(workspaceExample, `"mode":"current_checkout"`, `"mode":"current_checkout","baseRef":"main"`, 1), strings.Replace(workspaceExample, `"mode":"current_checkout"`, `"mode":"new_worktree"`, 1)} {
		if _, err := workflow.Decode([]byte(raw)); err == nil {
			t.Fatal("contradictory or incomplete checkout accepted")
		}
	}
}
