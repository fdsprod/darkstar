...
const workspaceExample = `{
 ...
 "spec":{
  ...
  "nodes":{
   ...
   "implement":{"type":"implementation","displayName":"Implement work item","entry":false,"terminal":false,"inputs":{"task":{"type":"task","from":"run.input.task"},"workspace":{"type":"workspace","from":"node.prepare.output.workspace"}},"outputs":{"changeset":{"type":"object"}},"implementation":{"taskInput":"task","workspaceInput":"workspace","instructions":"Implement the requested task in the connected workspace; preserve unrelated work. Do not publish."},"permissions":["process.run","workspace.write"],"transitions":[{"id":"implemented","to":"validate","kind":"normal"}]},
   "validate":{"type":"workspace_validate","displayName":"Validate workspace","entry":false,"terminal":true,"inputs":{"workspace":{"type":"workspace","from":"node.prepare.output.workspace"}},"outputs":{"validation":{"type":"object"}},"workspaceValidate":{"workspaceInput":"workspace","checks":[["git","diff","--check"]]},"transitions":[]}
  }
 }
}`
