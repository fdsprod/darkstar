...
func TestOpenCreatesAndRecordsFreshSchema(t *testing.T) {
	...
	if migrationCount != 35 {
		t.Errorf("migration count = %d, want 35", migrationCount)
	}
	if err := database.SQL().QueryRowContext(ctx,
...
func TestOpenIsIdempotent(t *testing.T) {
	...
	if count != 35 {
		t.Fatalf("migration count after reopen = %d, want 35", count)
	}
}
