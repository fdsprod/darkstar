...
func TestOpenCreatesAndRecordsFreshSchema(t *testing.T) {
	...
	if migrationCount != 31 {
		t.Errorf("migration count = %d, want 31", migrationCount)
	}
	if err := database.SQL().QueryRowContext(ctx,
...
func TestOpenIsIdempotent(t *testing.T) {
	...
	if count != 31 {
		t.Fatalf("migration count after reopen = %d, want 31", count)
	}
}
