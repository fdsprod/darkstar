...
func TestOpenCreatesAndRecordsFreshSchema(t *testing.T) {
	...
	if migrationCount != 22 {
		t.Errorf("migration count = %d, want 22", migrationCount)
	}
	if err := database.SQL().QueryRowContext(ctx,
...
func TestOpenIsIdempotent(t *testing.T) {
	...
	if count != 22 {
		t.Fatalf("migration count after reopen = %d, want 22", count)
	}
}
