...
func TestOpenCreatesAndRecordsFreshSchema(t *testing.T) {
	...
	if migrationCount != 29 {
		t.Errorf("migration count = %d, want 29", migrationCount)
	}
	if err := database.SQL().QueryRowContext(ctx,
...
func TestOpenIsIdempotent(t *testing.T) {
	...
	if count != 29 {
		t.Fatalf("migration count after reopen = %d, want 29", count)
	}
}
