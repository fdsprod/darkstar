...
func TestOpenCreatesAndRecordsFreshSchema(t *testing.T) {
	...
	if migrationCount != 32 {
		t.Errorf("migration count = %d, want 32", migrationCount)
	}
	if err := database.SQL().QueryRowContext(ctx,
...
func TestOpenIsIdempotent(t *testing.T) {
	...
	if count != 32 {
		t.Fatalf("migration count after reopen = %d, want 32", count)
	}
}
