...
func TestOpenCreatesAndRecordsFreshSchema(t *testing.T) {
	...
	wantTables := []string{
		...
		"node_projection", "point_dependencies", "point_projection", "project_projection", "queue_entries", "recovery_decisions", "run_projection", "run_workflow_snapshots", "schema_migrations", "story_projection", "work_item_projection", "workflow_versions",
	}
	...
	if migrationCount != 14 {
		t.Errorf("migration count = %d, want 14", migrationCount)
	}
	if err := database.SQL().QueryRowContext(ctx,
