...
func TestOpenCreatesAndRecordsFreshSchema(t *testing.T) {
	...
	if migrationCount != 25 {
		t.Errorf("migration count = %d, want 25", migrationCount)
	}
	if err := database.SQL().QueryRowContext(ctx,
...
func TestOpenIsIdempotent(t *testing.T) {
	...
	if count != 25 {
		t.Fatalf("migration count after reopen = %d, want 25", count)
	}
}
