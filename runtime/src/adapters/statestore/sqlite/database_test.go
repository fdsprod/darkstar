...
func TestOpenCreatesAndRecordsFreshSchema(t *testing.T) {
	...
	if migrationCount != 33 {
		t.Errorf("migration count = %d, want 33", migrationCount)
	}
	if err := database.SQL().QueryRowContext(ctx,
...
func TestOpenIsIdempotent(t *testing.T) {
	...
	if count != 33 {
		t.Fatalf("migration count after reopen = %d, want 33", count)
	}
}
