...
func TestOpenCreatesAndRecordsFreshSchema(t *testing.T) {
	...
	if migrationCount != 24 {
		t.Errorf("migration count = %d, want 24", migrationCount)
	}
	if err := database.SQL().QueryRowContext(ctx,
...
func TestOpenIsIdempotent(t *testing.T) {
	...
	if count != 24 {
		t.Fatalf("migration count after reopen = %d, want 24", count)
	}
}
