...
func TestOpenCreatesAndRecordsFreshSchema(t *testing.T) {
	...
	wantTables := []string{
		...
		"input_request_projection", "node_definitions", "node_projection", "point_dependencies", "point_projection", "project_projection", "provider_permission_projection", "queue_entries", "readiness_assessment_projection", "recovery_decisions", "run_execution_contexts", "run_projection", "run_workflow_snapshots", "schema_migrations", "story_projection", "work_item_projection", "workflow_versions",
	}
	...
	if migrationCount != 26 {
		t.Errorf("migration count = %d, want 26", migrationCount)
	}
	if err := database.SQL().QueryRowContext(ctx,
...
func TestOpenIsIdempotent(t *testing.T) {
	...
	if count != 26 {
		t.Fatalf("migration count after reopen = %d, want 26", count)
	}
}
