...
CREATE TABLE artifact_representations (
  ...
  metadata_json TEXT NOT NULL CHECK (json_valid(metadata_json) AND json_type(metadata_json) = 'object'),
  created_at TEXT NOT NULL,
  processor_digest TEXT NOT NULL DEFAULT '' CHECK (processor_digest = '' OR (length(processor_digest) = 64 AND processor_digest NOT GLOB '*[^0-9a-f]*')),
...
CREATE TABLE run_execution_contexts (
  ...
  created_at TEXT NOT NULL,
  updated_at TEXT NOT NULL,
  extension_pins_json TEXT NOT NULL DEFAULT '{}' CHECK (json_valid(extension_pins_json) AND json_type(extension_pins_json) = 'object'),
  provider_name TEXT NOT NULL DEFAULT '',
  prompt_snapshots_json TEXT NOT NULL DEFAULT '{}' CHECK (json_valid(prompt_snapshots_json) AND json_type(prompt_snapshots_json) = 'object')
) STRICT;
